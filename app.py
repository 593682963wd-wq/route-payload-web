...
"""航线载量分析 — Streamlit 网页版 UI。
风格对齐机场障碍物分析网页版（同一个紫色卡片 + 中文按钮）。
"""
from __future__ import annotations
...
.metric-card {
    background: #f6f1fb; border-left: 4px solid #6a3093;
    padding: .75rem 1rem; border-radius: 6px; margin: .25rem 0;
}
.small-muted { color: #888; font-size: .85rem; }
hr { border-color: #ece7f3; }
</style>
""",
    unsafe_allow_html=True,
)
...
st.title("✈️ 航线载量分析工具")
st.caption(f"批量解析 OFP TXT，按机场障碍物分析风格输出 Word 表格 · v{APP_VERSION}")
...
with st.sidebar:
    ...
    airports = _load_json("airports.json")
    aircraft = _load_json("aircraft.json")
    st.markdown(f"<div class='metric-card'>已映射机场：<b>{len(airports)}</b> 个</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='metric-card'>已映射机型：<b>{len(aircraft)}</b> 个</div>", unsafe_allow_html=True)
    st.markdown("<span class='small-muted'>映射文件位置：</span>", unsafe_allow_html=True)
    st.code(str(ROOT / "config"), language="text")
    with st.expander("📖 文件名规则"):
        st.markdown(
            "- 例： `306C ZSWX-ZWTL S07.txt`\n"
            "- 末尾 `S/N/W` → 南线/北线/W线\n"
            "- 末尾两位数字 → 月份"
        )
    with st.expander("🛠 自定义机场/机型映射"):
        if st.button("打开 airports.json"):
            st.code((ROOT / "config" / "airports.json").read_text(encoding="utf-8"))
        if st.button("打开 aircraft.json"):
            st.code((ROOT / "config" / "aircraft.json").read_text(encoding="utf-8"))

# ---------- 上传 ----------
st.subheader("1. 上传 TXT 文件")
files = st.file_uploader(
    "可一次拖入数百份 OFP TXT",
    type=["txt"],
    accept_multiple_files=True,
    help="留好性能位：一次性可处理 500+ 份 TXT。",
)

if not files:
    st.info("拖入或选择 OFP TXT 后开始解析。")
    st.stop()

st.success(f"已选择 {len(files)} 个文件")

# ---------- 解析 ----------
st.subheader("2. 解析结果预览")
...
if errors:
    with st.expander(f"⚠️ {len(errors)} 个文件解析失败", expanded=False):
        for name, err in errors:
            st.error(f"{name}: {err}")
...
if "docx_bytes" in st.session_state:
    with col2:
        ...
        st.caption(f"大小：{len(st.session_state['docx_bytes']) / 1024:.1f} KB")
